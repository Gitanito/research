...
file_put_contents("../wiki/index.html", $out);
    ...
    try {
        ...
        $startid = ($db->query("SELECT value FROM settings where name='makingstartpoint';"))->fetchArray()[0];
        ...
        while ($entry = $entries->fetchArray(SQLITE3_ASSOC)) {
            ...
            echo $ekey+$startid+1 . " von ". $entriescount. " : ".$intitle."\n";
            $db->exec("UPDATE settings SET value='".($startid + $ekey)."' WHERE name='makingstartpoint';");
                /*
            ...
            if (!strstr($entry['mylink'], '.pdf')) {
                ...
                foreach ($match_links as $kw => $lnk) {
                    $md5s[$kw] = md5($kw);
                    $preout = preg_replace("/" . preg_quote($kw, '/') . "/i", $md5s[$kw], $preout);
                }

                foreach ($match_links as $kw => $lnk) {
                    $preout = preg_replace("/" . preg_quote($md5s[$kw], '/') . "/i", "<a href='" . $lnk . "'>" . $kw . "</a>", $preout);
                }
                $out = $preout;
        ...
        echo "\n\nWriting WordCloud.\n";
        $s = 0;
        foreach ($_wordcloud_index as $keyword => $subs ) {
            $stmt = $db->prepare("INSERT INTO wordcloud (name,value) values (:name,:value);");
            $stmt->bindValue(':name', $keyword, SQLITE3_TEXT);
            $stmt->bindValue(':value', json_encode($subs), SQLITE3_TEXT);
            $stmt->execute();
            echo $s++ . " of " . sizeof($_wordcloud_index) . " written.\n";
        }
